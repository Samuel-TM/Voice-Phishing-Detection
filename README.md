...
```text
ML/
├── ChineseBERTModel/     # Text risk classification module
├── audio_risk_detection/ # Audio risk detection module
...
The multimodal system processes incoming audio through **speaker diarization**, segmenting by speaker turns. Each utterance is then analyzed in parallel via **text analysis (Chinese BERT)** and **voice analysis (CNN–BiLSTM)**. The resulting scores are fused to produce the final phishing decision.


### Text Analysis Model (Chinese BERT-based)
...
Text input is embedded with Chinese BERT and processed through a **Self-Attention–based Transformer Encoder**. Attention visualization highlights key tokens, and the output is passed through Fully Connected Layers with Softmax to generate phishing/normal probabilities.
